        ...
        {       /* movep short_addressing , ea */
            ...
            else
            {
                if( new_src_addr >= 0xffff80 && new_src_addr <= 0xffffbf)
                {
                    if(src_xory==0)
                    {			/* x mem */
                        inst_code.w0=0x74000|(dir<<15)|(dst_ea->w0<<8)|(dst_xory<<6)|(new_src_addr-0xffff80);
                    }
                    else	
                    {           /* y mem */
                        inst_code.w0=0x70080|(dir<<15)|(dst_ea->w0<<8)|(dst_xory<<6)|(new_src_addr-0xffff80);
                    }
                }
                else
                {
...
void GenRep2(int exp)
{
	IN_PASS1
	IN_PASS2
		if(exp>0xfff){ yyerror(ERROR_9); exp=0;}
...
{	
    IN_PASS1
    IN_PASS2
        if(src_reg1==dst_reg1) { yyerror(ERROR_8); src_reg1=0; }
        src_reg1=ddddd_2_JJJ(src_reg1);
        if(src_reg1==-1){ yyerror(ERROR_16); src_reg1=0; }
        dst_reg1=ddddd_2_d_dst(dst_reg1);
        src_reg2=ddddd_2_RRR(src_reg2);
        if(src_reg2==-1){ yyerror(ERROR_16); src_reg2=0; }
        dst_reg2=ddddd_2_RRR(dst_reg2);
        if(dst_reg2==-1){ yyerror(ERROR_8); dst_reg2=0; }
...
void GenTfr(uint src_reg,uint dst_reg,bcode *par_move)
{
    IN_PASS1
        inst_code.sflag=par_move->sflag;
    IN_PASS2
        if(src_reg==dst_reg){ yyerror(ERROR_8); src_reg=0; }
        src_reg=ddddd_2_JJJ2(src_reg);
        if(src_reg==-1){ yyerror(ERROR_16); src_reg=0; }
        dst_reg=ddddd_2_d_dst(dst_reg);
